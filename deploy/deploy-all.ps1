...
"@
    ...
    return [PSCustomObject]@{
        ...
        SshIdentityKey  = if ($cfg.sshIdentityKey) { [string]$cfg.sshIdentityKey } else { 'gisebs_deploy' }
    }
}
...
function Get-AppSettingsFromProject {
    ...
    $provider = [string]$json.Database.Provider
    $schema = [string]$json.Database.Schema
    if ([string]::IsNullOrWhiteSpace($provider) -or [string]::IsNullOrWhiteSpace($schema)) {
        throw "Database:Provider ou Database:Schema manquant dans appsettings.json"
    }
...
function New-SystemdServiceContent {
    ...
    $providerEscaped = $DatabaseProvider -replace '\\', '\\' -replace '"', '\"'
    $schemaEscaped = $DatabaseSchema -replace '\\', '\\' -replace '"', '\"'
...
Environment=ASPNETCORE_URLS=http://localhost:$($Project.ListenPort)
Environment="ConnectionStrings__DefaultConnection=$connEscaped"
Environment="Database__Provider=$providerEscaped"
Environment="Database__Schema=$schemaEscaped"
Restart=always
RestartSec=5
KillSignal=SIGINT
...
Write-Host "  Provider=$($AppSettings.DatabaseProvider), Schema=$($AppSettings.DatabaseSchema)" -ForegroundColor DarkGray
...
$serviceContent = New-SystemdServiceContent -ConnString $ConnectionString -DatabaseProvider $AppSettings.DatabaseProvider -DatabaseSchema $AppSettings.DatabaseSchema
